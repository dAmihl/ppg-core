...
namespace PPG {
	...
	Puzzle* PuzzleGenerator::generatePuzzle(ObjVec objects, EventVec events, RuleVec rules)
	{
	...
	void PuzzleGenerator::removeNodeFromList(Node* N, NodeVec& nodes) {
	...
	*/
	...
	void PuzzleGenerator::cleanupNodes(Puzzle* P) {
		...
		for (NodeVec::iterator it = nodes.begin(); it != nodes.end(); ++it) {
			if (!R.hasFollowingNode(*it) && !R.hasPrecedingNode(*it)) {
				for (NodeVec::iterator find = nodes.begin(); find != nodes.end(); ++find) {
					if (*find != *it && PuzzleGeneratorHelper::checkEquality(*it, *find)) {
	...
	*/
	PuzzleRelation* PuzzleGenerator::simpleGenerateRelation(NodeVec nodes)
	{
		...
		for (std::vector<Node*>::iterator it = nodes.begin(); it != nodes.end() - 1; ++it) {
			...
			Pair<Node*, Node*> pair = PuzzleRelation::makePuzzlePair(N1, N2);
			if (!PuzzleGeneratorHelper::checkEquality(N1, N2)) {
				rel->addPair(pair);
				if (PuzzleGeneratorHelper::checkCreatesCircularDependency(pair, rel) || PuzzleGeneratorHelper::checkCreatesExclusiveDependency(pair, rel) ||
					PuzzleGeneratorHelper::checkMetaEqualOccurance(pair, rel)) {
	...
	*/
	PuzzleRelation* PuzzleGenerator::generateRelation(NodeVec nodes, RuleVec rules)
	{
		PuzzleRelation* rel = new PuzzleRelation();
		...
		*/
		...
		for (std::vector<Node*>::iterator it = nodes.begin(); it != nodes.end(); ++it) {
			...
			*/
			NodeVec compList = PuzzleGeneratorHelper::filterCompatibleNodes(N1, rel, nodes, rules);

			// returns nullptr if list is empty
			Node* N2 = PuzzleRandomizer::getRandomNodeFromList(compList);
	...
	PuzzleRelation* PuzzleGenerator::generateRelationExperimental(Puzzle* P, NodeVec nodes, RuleVec rules)
	{
		PuzzleRelation* rel = new PuzzleRelation();

		NodeVec nodesInGraph;

		Node* NStart = PuzzleRandomizer::getRandomNodeFromList(nodes);
		...
		for (std::vector<Node*>::iterator it = nodes.begin(); it != nodes.end(); ++it) {
			...
			* checks for basic rules (exclusive dependency, equality, circular dependency etc) and for custom rules
			*/
			NodeVec compList = PuzzleGeneratorHelper::filterCompatibleNodes(N1, rel, tmpNodes, rules);
	...
	NodeVec PuzzleGenerator::generateNodes(ObjVec objects)
	{

		NodeVec nodes;

		for (int i = 0; i < this->NUM_NODES; i++) {
			PuzzleObject* obj = PuzzleRandomizer::getRandomFromList(objects);
	...
	*/
	void PuzzleGenerator::initializeActivePropertyOnNodes(Puzzle* P)
	{
