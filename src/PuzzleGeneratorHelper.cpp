...
namespace PPG {
	...
	*/
	bool PuzzleGeneratorHelper::checkCreatesCircularDependency(Pair<Node*, Node*> P, PuzzleRelation* R)
	{
	...
	*/
	bool PuzzleGeneratorHelper::checkCreatesExclusiveDependency(Pair<Node*, Node*> P, PuzzleRelation* R)
	{
	...
	*/
	bool PuzzleGeneratorHelper::findNodeSequential(Node* N, Pair<Node*, Node*> start, PuzzleRelation* R)
	{
	...
	bool PuzzleGeneratorHelper::checkEquality(Node* N1, Node* N2)
	{
	...
	*/
	...
	bool PuzzleGeneratorHelper::checkMetaEqualOccuranceByNode(Node* N, PuzzleRelation* R) {
	...
	bool PuzzleGeneratorHelper::checkMetaEqualOccurance(Pair<Node*, Node*> P, PuzzleRelation* R)
	{
		bool result = false;
	...
	*
	*/
	NodeVec PuzzleGeneratorHelper::filterCompatibleNodes(Node* N, PuzzleRelation* R, NodeVec nodes, RuleVec rules)
	{
		NodeVec compatibles;
	...
	*/
	...
	bool PuzzleGeneratorHelper::checkCompatibilityBasicRules(Node* S, Node* N, PuzzleRelation* R)
	{
		Pair<Node*, Node*> pair = PuzzleRelation::makePuzzlePair(S, N);
	...
	*/
	bool PuzzleGeneratorHelper::checkCompatibilityCustomRules(NodeVec nodes, Node* S, Node* N, PuzzleRelation* R, RuleVec rules)
	{
		for (RuleVec::iterator r = rules.begin(); r != rules.end(); ++r) {
			...
			bool (*FN) (NodeVec, Node*, Node*, PuzzleRelation*, PuzzleRule, bool) = NULL;
			bool strict = false;
	...
	*/
	bool PuzzleGeneratorHelper::checkCompatibilityRuleTypeAfter(NodeVec nodes, Node* S, Node* N, PuzzleRelation* R, PuzzleRule rule, bool isStrict)
	{
		/* */
		...
		NodeVec existingRHS = R->findNodesByPattern(nodes, rhsO, rhsS, PuzzleGeneratorHelper::isRuleObjectEqual, PuzzleGeneratorHelper::isRuleStateEqual);
		NodeVec existingLHS = R->findNodesByPattern(nodes, lhsO, lhsS, PuzzleGeneratorHelper::isRuleObjectEqual, PuzzleGeneratorHelper::isRuleStateEqual);
		...
		// if N is the LHS, then S has to either be RHS (strict) or has RHS as a preceding node
			...
			//return true;
			PuzzleRule::EPuzzleRuleType tmpType = isStrict ? PuzzleRule::EPuzzleRuleType::STRICT_BEFORE : PuzzleRule::EPuzzleRuleType::BEFORE;
	...
	*	False = INCOMPATIBLE!
	*/
	bool PuzzleGeneratorHelper::checkCompatibilityRuleTypeBefore(NodeVec nodes, Node* S, Node* N, PuzzleRelation* R, PuzzleRule rule, bool isStrict)
	{
		/* */
		PuzzleObject* lhsO = rule.getLeftHandSideObject();
		PuzzleState* lhsS = rule.getLeftHandSideState();
	...
	bool PuzzleGeneratorHelper::isRuleNodeEqual(Node* N, PuzzleObject* ruleObject, PuzzleState* ruleState)
	{
